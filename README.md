...
[큐레이터 (나)]
       │
       ▼
[Streamlit Admin]  ← 가게 추가 (Google Maps URL 입력)
       │
       ├── Google Places API  → 가게 정보 자동 수집 (이름, 주소, 영업시간, 평점, 리뷰 등)
       │
       ├── Claude Haiku       → 메타데이터 자동 생성 (what_to_order, good_for, spice_level)
       ├── Claude Sonnet      → 영어 블로그 포스팅 생성 (SEO/GEO 최적화)
       │
